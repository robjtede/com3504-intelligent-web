...
var getTweets = function (querySet, callback) {
...
var insertTweetSingle = function (tweet) {
  var connection = getConnection();
  connection.connect();
...
module.exports.addSearch = function (termPlayer, termTeam, termAuthor, isAnd, callback) {
...
module.exports.getSearch = function (querySet, callback) {
  // Remove spaces from queries, and split into regexes by
  // replacing commas with pikes (|).
  // Also remove '@' from author, as this is implied by comparing to 'Author'.
  var regexPlayer = querySet.player.replace(/\s+/g, '').replace(/,/g, '|');
  ...
  */
  ...
  var sqlQuery = 'SELECT * FROM searches WHERE player REGEXP (' +
    connection.escape(regexPlayer) + ') AND team REGEXP (' +
      connection.escape(regexTeam) + ') AND author REGEXP (' +
        connection.escape(regexAuthor) + ')';
  // console.log(sqlQuery);
  connection.query({
    sql: sqlQuery,
    timeout: 40000 // 40s
